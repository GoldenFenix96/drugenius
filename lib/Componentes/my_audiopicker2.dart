...
class AudioPickerWidget2 extends StatefulWidget {
  ...
  const AudioPickerWidget2({Key? key, required this.updateAudios}) : super(key: key);

  @override
  _AudioPickerWidgetState2 createState() => _AudioPickerWidgetState2();
}
...
class _AudioPickerWidgetState2 extends State<AudioPickerWidget2> {
  ...
  Future<void> _pickAudioFromGallery() async {
    final returnedAudio = await FilePicker.platform.pickFiles(type: FileType.audio);
            ...
            if (_selectedAudio.isNotEmpty)
              ListView.builder( 
                ...
                itemBuilder: (context, index) => ListTile(
                  title: Text(_selectedAudio[index]!.path.split('/').last),  // Display audio filename
